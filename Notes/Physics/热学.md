...
### 泻流
速度在 $v_x \sim v_x + \text{d}v_x$、$v_y \sim v_y + \text{d}v_y$、$v_z \sim v_z + \text{d}v_z$ 之间，单位时间内碰到容器壁面积为 $\text{d}A$ 的小孔的分子数目为：
$$ nv_x \text{d}A F (v_x, v_y, v_z) \text{d}v_x \text{d}v_y \text{d}v_z $$
单位时间碰到容器壁单位面积上的总分子数目为：

$$ N = \int_{0}^{\infty} v_y \int_{0}^{\infty} dv_z \int_{0}^{\infty} dv_x \, m_e F (v_x, v_y, v_z) \frac{1}{4} dV $$